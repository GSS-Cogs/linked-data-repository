...
def get_store(store_type_label: str) -> (BaseStore):
    ...
    try:
        ...
        raise KeyError(f'No store of type {store_type_label} could be identified.') from err
    

@given(u'I\'m using a store of type "{wanted_store}"')
def step_impl(context, wanted_store):
    context.store = get_store(wanted_store)
    context.store.setup()

@given(u'I get metadata for a resource identified by "{resource_id}"')
def step_impl(context, resource_id):
    context.metadata = context.store.get_resource(resource_id)

@then(u'"{count}" fields are returned')
def step_impl(context, count):
    got = len(context.metadata)
    expected = int(count)
    assert got == expected, f'Got {got} field, expected {expected}.'

@then(u'there are "{count}" records with fields that match')
def step_impl(context, count):
    filter_kwargs = {}
    ...
    assert len(matched_records) == int(count), \
        f'Expected {int(count)} records, got {len(matched_records)}'
        
@given(u'I update a resource identified by "{resource_id}" with')
def step_impl(context, resource_id):
    for row in context.table:
        context.store.upsert_resource_field(resource_id, row[0], row[1])
        
@given(u'I create a new resource for the graph identifier "{graph_identifier}"')
