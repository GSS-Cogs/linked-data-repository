...
# Note - might be better of as an env var? Wouldn't need a keyword or default then
...
def get_config(logger, path: Union[str, Path] = DEFAULT_PATH):
    ...
    assert len(config["ENV"].keys()) == 0, ('The configuration.ini section ENV must'
        ' always be blank')
    ...
    for oaev_label in ['OAUTH_DOMAIN', 'OAUTH_CLIENT_ID', 'OAUTH_CLIENT_SECRET',
            'OAUTH_REDIRECT_URI']:
